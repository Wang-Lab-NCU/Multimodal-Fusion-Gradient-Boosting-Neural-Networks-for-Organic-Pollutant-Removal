...
# shap.dependence_plot('Trivalent cations (mmol/L)', attributions_numpy, x_train_np, feature_names=features,
#                      interaction_index='Pressure (MPa)')
# shap.dependence_plot('Divalent cations (mmol/L)', attributions_numpy, x_train_np, feature_names=features,
#                      interaction_index='Pressure (MPa)')
# shap.dependence_plot('Monovalent cations (mmol/L)', attributions_numpy, x_train_np, feature_names=features,
#                      interaction_index='Pressure (MPa)')
#
# shap.dependence_plot(('Trivalent cations (mmol/L)', 'Pressure (MPa)'), attributions_numpy, x_train_np
#                      )
# shap.dependence_plot(('Divalent cations (mmol/L)', 'Pressure (MPa)'), attributions_numpy, x_train_np
#                      )
# shap.dependence_plot(('Monovalent cations (mmol/L)', 'Pressure (MPa)'), attributions_numpy, x_train_np
#                      )
#
...
# shap.summary_plot(shap_values, x_train.numpy(),feature_names=features)

# shap.dependence_plot('Trivalent cations (mmol/L)', shap_values, x_train_np, feature_names=features,
#                      interaction_index='Pressure (MPa)', show=False)
# plt.savefig("Figure_pre_cation_shap.tif", format="tif", dpi=400)
#
# plt.show()
# shap.dependence_plot('Divalent cations (mmol/L)', shap_values, x_train_np, feature_names=features,
#                      interaction_index='Pressure (MPa)')
# shap.dependence_plot('Monovalent cations (mmol/L)', shap_values, x_train_np, feature_names=features,
#                      interaction_index='Pressure (MPa)')

# shap.dependence_plot('water flux(LMH)', shap_values, x_train_np, feature_names=features, interaction_index='Pressure (MPa)',show=False)
#                       )
# shap.dependence_plot('water flux(LMH)', shap_values, x_train_np, feature_names=features, interaction_index='Pore size(nm)',show=False)
#
# shap.dependence_plot('water flux(LMH)', shap_values, x_train_np, feature_names=features, interaction_index='Temperature (˚C)',show=False)


# shap.dependence_plot('Pore size(nm)', shap_values, x_train_np, feature_names=features, interaction_index='water flux(LMH)')
# plt.savefig("Figure_pre_cation_shap.tif", format="tif", dpi=300)
#
# plt.show()
...
temp = 'water flux(LMH)'
interact_out = pdp.pdp_isolate(model=mymodel, dataset=x_train_np, model_features=features,
                                feature=temp)
# Plot the feature interaction
pdp.pdp_plot(pdp_isolate_out=interact_out, feature_name=temp,
             plot_params={'title_fontsize': 13}, figsize=(8.3, 7))
