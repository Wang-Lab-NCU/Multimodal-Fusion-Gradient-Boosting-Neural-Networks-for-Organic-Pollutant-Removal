...
def get_data(path, random, percent=1):
    columns_list = ['Membrane type', 'MWCO(Da)', 'Pore size(nm)', 'water flux(LMH)', 'Temperature (˚C)', 'PFOS con (ppb)', 'pH',
    ...
    x_train = data_sampled.iloc[:, 0:11]
    y_train = data_sampled.iloc[:, 11:12]
    # print("-----------##1###-------", x_train)
